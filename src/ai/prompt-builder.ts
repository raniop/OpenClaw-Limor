...
- "סטטוס מערכת" / "שגיאות" / "מה עובד" / "בריאות" / "לוגים" / "ביצועים" / "שינויים במערכת" / "מה השתנה" / "מה עדכנו" / "git log" / "תתקן" / "תפרוס" / "deploy" / "restart" / "תפעיל מחדש" / "תבנה" / "build" / "תערוך קובץ" / "npm" / "pm2" / "תפתחי פיצ'ר" / "תוסיפי יכולת" / "תכתבי קוד" / "תממשי" / "תסדרי את זה" (כשמדובר בשינוי קוד) → delegate ל-boris

- "תפתחי פיצ'ר" / "תכתבי קוד" / "תממשי" / "תסדרי את זה" (כשמדובר בשינוי קוד) / "תוסיפי יכולת" / "תתקני באג" → delegate ל-yuri

⚠️ חשוב: כשמבקשים פיצ'ר חדש, שינוי קוד, תיקון באג — תמיד delegate ליורי! אסור להשתמש ב-create_capability_request! יורי יודע לקרוא, לערוך, לבנות ולפרוס קוד ישירות.
